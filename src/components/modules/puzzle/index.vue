<template>
  <div class="puzzle-module">
    <div v-if="sub === 1" class="puzzle-main">
      <div class="puzzle-tabs">
        <div class="tab-item" :class="{ active: activeTab === 'grid' }" @click="activeTab = 'grid'">
          网格
        </div>
        <div class="tab-item" :class="{ active: activeTab === 'join' }" @click="activeTab = 'join'">
          拼接
        </div>
      </div>
      ...
      <div>
        ...
        <div>
          ...
          <div v-if="showControlPanel" class="control-panel">
            <div class="control-group">
              ...
              <div class="size-controls">
                <div class="input-wrapper">
                  <input type="number" v-model.number="puzzleSettings.width" :min="min" :max="max" class="ie-input"
                    @change="updateSize('width')">
                  <span class="suffix">W</span>
                </div>

                <div class="link-icon" title="锁定宽高比" @click="toggleRatioLock">
                  <svg v-if="puzzleSettings.lockRatio" width="16" height="16" viewBox="0 0 24 24" fill="none"
                ...
                <div class="input-wrapper">
                  <input type="number" v-model.number="puzzleSettings.height" :min="min" :max="max" class="ie-input"
            ...
            <div class="control-group">
              <label>边框</label>
              <input v-model.number="puzzleSettings.padding" type="range" min="0" max="100" step="1"
                @change="updateStyle" />
            ...
            <div class="control-group">
              <label>间距</label>
              <input type="range" v-model.number="puzzleSettings.spacing" min="0" max="100" step="1"
                @change="updateStyle" />
            ...
            <div class="control-group">
              <label>拼图背景颜色</label>
              <input type="color" class="color" v-model="puzzleSettings.bgColor" @input="updateStyle" />
            </div>
        ...
        <div v-for="(group, count) in filteredTemplates" :key="count" class="grid-group">
          ...
          <div class="grid-templates-row">
            <div v-for="template in group" :key="template.id" :style="template.wrapStyle" class="grid-template"
              ...
              <div v-for="(gridArea, i) in template.gridAreas" :key="i" class="grid-cell"
                :style="`grid-area: ${gridArea};`"></div>
            </div>
          </div>
        </div>
    ...
    <div v-if="sub === 3" class="p-20">
      <div class="operate-buttons">
        <button class="ie-btn full" @click="triggerReplace">替换图片</button>
        <button v-if="currentImgCount > 1" class="ie-btn btn-danger" style="margin-left: 8px;"
    ...
    <Modal v-model="showSaveModal" @confirm="handleConfirmSave" @cancel="showSaveModal = false"
